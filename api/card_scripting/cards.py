...
cardTexts = {
    ...
    'bureaucrat': '#gain(#fromStore(silver), deck); #attack(`#setText(Choose a victory card to put on top of your deck); options=#getSubset(#getHand(), #makeArray(type, =, victory)); #toDeck(#chooseSubset($options, 1, #false())); #cond(#eval(#count($options), = , 0), #reveal(#getHand(), Player reveals their hand with no victory cards:))`)',
    ...
    'moneylender': 'x=#getSubset(#getHand(), #makeArray(name, =, copper)); #setText(Choose up to one Copper to trash); toTrash=#chooseSubset($x, 1, #true()); willTrash=#false(); #cond(#eval(#count($toTrash), >, 0), willTrash=#true()); #cond($willTrash, #trash($toTrash)); #cond($willTrash, #changeCoins(3)); &resetText',
